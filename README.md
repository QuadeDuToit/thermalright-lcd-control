...
## 🔄 Rotation Feature Enhancement

This fork adds **display rotation controls** for users who need to flip or rotate their LCD display orientation.

### New Features Added:

#### GUI Rotation Controls
- **Rotation Dropdown**: Select from 0°, 90°, 180°, or 270° rotation in the GUI
- **Quick Flip Button**: One-click toggle between normal and upside-down (180°)
- **Auto-loading**: GUI remembers and displays current rotation setting
- **Live Updates**: Changes apply immediately with automatic service restart

#### Command Line Tools
- **`lcd-flip`**: Simple toggle script between normal and flipped
- **`lcd-rotate`**: Advanced control with options: `normal`, `flip`, `left`, `right`, `toggle`, `status`
- **`lcd-rotation-gui`**: Standalone rotation control GUI

#### Configuration Support
- **YAML Config**: Add `rotation: 180` to your config file under `display:` section
- **Multiple Angles**: Supports 0°, 90°, 180°, 270° rotation values
- **Persistent Settings**: Rotation preferences saved across restarts

### Installation of Rotation Feature

The rotation controls are automatically available after installing this fork. No additional setup required!

### Usage Examples

```bash
# Command line usage
lcd-flip                    # Toggle between normal and upside-down
lcd-rotate flip            # Force upside-down orientation  
lcd-rotate normal          # Force normal orientation
lcd-rotate status          # Check current rotation

# GUI usage
thermalright-lcd-control   # Open GUI and look for "Display Rotation" section
```

### Technical Details

- **Device Support**: Tested with Thermalright Frozen Vision 360 ARGB (87AD:70DB)
- **Implementation**: PIL/Pillow image rotation in rendering pipeline
- **Performance**: Minimal overhead, rotation applied per-frame
- **Compatibility**: Maintains full compatibility with original features
