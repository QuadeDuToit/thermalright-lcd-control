...
class ControlsManager:
    ...
    def _create_rotation_controls(self) -> QGroupBox:
        """Create display rotation controls"""
        ...
        self.rotation_combo.addItems(["0° (Normal)", "90° (Left)", "180° (Flipped)", "270° (Right)"])
        self.rotation_combo.setCurrentIndex(0)  # Default to normal
        
        # Load current rotation from config
        try:
        ...
        # Add flip button for quick toggle
        flip_btn = QPushButton("Quick Flip")
        flip_btn.setToolTip("Toggle between normal and upside-down")
        flip_btn.clicked.connect(self._on_quick_flip)
        flip_btn.setFixedSize(80, 30)
    ...
    def _get_current_rotation(self):
        ...
        try:
            ...
            return config.get('display', {}).get('rotation', 0)
        except:
            return 0
    ...
    def _set_rotation(self, degrees):
        ...
        try:
            ...
            if 'display' not in config:
                config['display'] = {}
            config['display']['rotation'] = degrees
